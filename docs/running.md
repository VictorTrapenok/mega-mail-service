...
Everything needed to deploy the bench and take a measurement. The summary and the findings
are in the [README](../README.md); what the numbers mean is in
...
`ghcr.io/victortrapenok/mega-mail-service/postal:src-<digest>`. That `src-` tag is the same string the report
prints as "Source tree", which is what makes a handed-over image checkable rather than
merely asserted. See
...
[custom-builds.md](custom-builds.md). What we have changed in Postal so far is in
...
(see [postal-internals.md](postal-internals.md)). Hence it makes sense to measure

# Engineering log: benchmarking Postal, and every wrong turn

The long-form record of how the Postal benchmark was built and how its numbers were earned —
including the measurements that turned out to be wrong and how each was caught. It is kept
because the mistakes are the useful part: most of them are traps that anyone load testing a
mail server will walk into (a generator that is slower than the system under test, a limit
that silently limits nothing, an ingress rate mistaken for throughput).

The short list of what was done is in [CHANGELOG.md](../CHANGELOG.md), the findings in
[RESULTS.md](../RESULTS.md), our changes to Postal in [optimisations.md](optimisations.md),
and possible next steps in [roadmap.md](../roadmap.md).
...
- **A fresh pair of reference runs on the current profiles.** With `send_limit` set and
  ...
  that is now in [roadmap.md](../roadmap.md). The capacity arithmetic survived: 28 sending addresses for the

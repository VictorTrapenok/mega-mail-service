...
горячих запросов воркера не покрыт индексом, поэтому стоимость обработки письма
зависит от того, сколько строк лежит в таблице и насколько редок его `batch_key`.
Измерение на пустой очереди относится к случаю, которого в проде не бывает.
